...
/**
 * 根据地区编码取得地区名称
 * @param paramCode：地区编码
 * @returns {string}：地区名称
 */
function getAreaNameFunc(paramCode) {

    // 临时变量：市名称
    var tempCityNameStr = '';
    // 临时变量：省名称
    var tempProvinceNameStr = '';
    ...
    for (var i = 0; i < Areas.length; i++){
        // 临时变量：省编码
        var tempProvinceCodeStr;
        // 根据参数取得对应城市
        if(Number(paramCode) == Number(Areas[i].code)){
            // 取得市名称
            tempCityNameStr = Areas[i].name;
            // 取得省编码
            tempProvinceCodeStr = Areas[i].parentCode;
            // 根据地区编码，检索省
            if (tempProvinceCodeStr != '' && tempProvinceCodeStr != null){
                tempProvinceNameStr = getAreaNameFunc(tempProvinceCodeStr);
            }

            return tempProvinceNameStr + tempCityNameStr;
        }
    }
...
// 获取选择的省下的所有城市的id
function getCityIdsFunc(provinceCode) {

    var cityArr=new Array();

    for(var i = 0; i < Areas.length; i++){
        if(Number(provinceCode) == Number(Areas[i].parentCode)){
            cityArr.push("'"+Areas[i].code+"'");
        }
    }
    cityArr.push("'"+provinceCode+"'");
    var cityStr = cityArr.join(",");
    return cityStr;
}