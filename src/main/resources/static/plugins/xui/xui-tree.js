	...
	layer.prompt(
		{
			title: "修改组名",
			success: function (lo) {
				lo.find(".layui-layer-input").val(item.name);
			}
		},
		function (data,i) {
		//确认后修改当前对象的name属性
		item.name = data;
		//然后重新渲染树形菜单
		new Xui_tree(options);
		//暴露接口
		if(that.options.change instanceof Function) that.options.change(item, data);
		//关闭layer;
		layer.close(i);
	});
    });
